...
app.get("/api/gpu", async (req, res) => {
  for (const w of WORKERS) {
    try {
      let r;
      try {
        r = await axios.get(`${w}/gpu`, { timeout: 2500 });
      } catch {
        r = await axios.get(`${w}/health`, { timeout: 2500 });
      }
...
app.post("/api/jobs/submit", upload.array("files"), async (req, res) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    form.append("file", fs.createReadStream(f.path), f.originalname); // preserves .nii vs .nii.gz
    ...
    try {
      ...
      // Clean up local temp file after sending to Python worker
...
app.get("/api/jobs/:id/overlay", async (req, res) => {
  try {
    ...
    res.status(404).json({ error: "overlay not available yet" });
  }
});
...
app.get("/api/jobs/:id/image", async (req, res) => {
  try {
    ...
    // forward content-disposition if present to handle .nii vs .nii.gz dynamically
...
app.get("/api/jobs/:id/mask", async (req, res) => {
  try {
    ...
    res.status(404).json({ error: "mask not available yet" });
  }
});

// NEW: meta + slice proxy (for interactive 3D exploration)
...
app.post("/api/jobs/:id/xai/analyze", async (req, res) => {
  try {
    const w = workerFor(req.params.id);
    const qs = new URLSearchParams(req.query).toString();
    const r = await axios.post(`${w}/xai/analyze/${req.params.id}${qs ? "?" + qs : ""}`, {}, { timeout: 10000 });
    res.json(r.data);
...
app.get("/api/jobs/:id/xai/status", async (req, res) => {
  try {
    const w = workerFor(req.params.id);
    const r = await axios.get(`${w}/xai/status/${req.params.id}`, { timeout: 5000 });
    res.json(r.data);
  } catch (e) { res.status(500).json({ error: String(e?.message || e) }); }
});
...
app.get("/api/jobs/:id/xai/metrics", async (req, res) => {
  try {
    const w = workerFor(req.params.id);
    const r = await axios.get(`${w}/xai/metrics/${req.params.id}`, { timeout: 5000 });
    res.json(r.data);
  } catch (e) { res.status(500).json({ error: String(e?.message || e) }); }
});
...
app.get("/api/jobs/:id/xai/slice", async (req, res) => {
  try {
    const w = workerFor(req.params.id);
    const qs = new URLSearchParams(req.query).toString();
    const r = await axios.get(`${w}/xai/slice/${req.params.id}${qs ? "?" + qs : ""}`,
...
app.get("/api/jobs/:id/xai/download/:kind", async (req, res) => {
  try {
    ...
    res.setHeader("Content-Disposition", `attachment; filename="${req.params.id}_${req.params.kind}.nii.gz"`);
    res.setHeader("Cache-Control", "no-store");
    res.send(Buffer.from(r.data));
  } catch (e) { res.status(500).json({ error: String(e?.message || e) }); }
});
...
app.get("/api/jobs/:id/xai/probe", async (req, res) => {
  try {
    const w = workerFor(req.params.id);
    const qs = new URLSearchParams(req.query).toString();
    const r = await axios.get(`${w}/xai/probe/${req.params.id}${qs ? "?" + qs : ""}`, { timeout: 30000 });
    res.json(r.data);
  } catch (e) { res.status(500).json({ error: String(e?.message || e) }); }
});
...
app.get("/api/xai/colormaps", async (req, res) => {
  try {
    const r = await axios.get(`${WORKERS[0]}/xai/colormaps`, { timeout: 5000 });
    res.json(r.data);
  } catch { res.json(["jet", "hot", "cool", "viridis", "plasma", "inferno", "RdBu_r"]); }
});
...
app.get("/api/jobs/:id/xai/volume/:kind", async (req, res) => {
  try {
    ...
    if (r.headers["x-shape"]) res.setHeader("X-Shape", r.headers["x-shape"]);
    res.setHeader("Cache-Control", "no-store");
    res.send(Buffer.from(r.data));
  } catch (e) { res.status(500).json({ error: String(e?.message || e) }); }
});
