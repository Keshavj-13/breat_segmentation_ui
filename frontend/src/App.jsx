      ...
      <div className="container">
        ...
        <div className="header">
          <div>
            ...
            <div className="sub">Automated 3D tumor segmentation for research-grade volumetric analysis.</div>
          </div>
          <div className="row" style={{ gap: '1rem' }}>
            <button className="btn ghost" onClick={refreshGpu}><RefreshCw size={16} style={{marginRight: '6px'}}/> Refresh Compute Status</button>
        ...
        <div className="grid">
          ...
          <div className="card">
            <div className="cardHead">
              <h2 className="h2" style={{display: 'flex', alignItems: 'center', gap: '8px'}}><Activity size={20} color="var(--accent-cyan)"/> Segmentation Submission</h2>
            </div>
            ...
            >
              ...
              <div style={{fontWeight: 600, fontSize: '1.1rem', marginBottom: '0.25rem'}}>Upload Imaging Volumes</div>
            ...
            {submitting && uploadProgress > 0 && uploadProgress < 100 && (
              <div style={{marginBottom: '1rem'}}>
                ...
                <div style={{ width: '100%', backgroundColor: 'var(--surface-alt)', border: '1px solid var(--border)', height: '8px', borderRadius: '4px', overflow: 'hidden' }}>
                  <div style={{ width: `${uploadProgress}%`, backgroundColor: 'var(--blue)', height: '100%', transition: 'width 0.2s' }} />
                </div>
              </div>
            )}
          ...
          <div className="card">
            <div className="cardHead">
              <h2 className="h2" style={{display: 'flex', alignItems: 'center', gap: '8px'}}><Cpu size={20} color="var(--accent-indigo)"/> Compute Status</h2>
            </div>
            {gpuErr ? (
              <div className="warn" style={{display: 'flex', gap: '10px'}}><AlertCircle size={20}/> Connection error: {gpuErr}</div>
            ) : (
        ...
        <div className="card" style={{ marginTop: '1.5rem' }}>
          <div className="cardHead">
            <h2 className="h2" style={{display: 'flex', alignItems: 'center', gap: '8px'}}><Database size={20}/> Case Processing Results</h2>
            <div className="pill" style={{borderColor: 'var(--border)'}}>{jobs.length} cases processed</div>
          </div>
                ...
                <div className="job" key={j.job_id}>
                  <div className="jobTop">
                    <div>
                      ...
                      <div className="muted small" style={{fontFamily: 'monospace'}}>Case ID: {j.job_id.substring(0,8)}... Node: {j.routed_to || "Compute Node"}</div>
                    </div>
                  ...
                  {j.status === "FAILED" ? <div className="warn" style={{display: 'flex', gap: '8px'}}><AlertCircle size={16}/> {j.error || "Pipeline error"}</div> : null}

                  {j.status === "DONE" && j.result ? (
                    <>
                      <div className="row" style={{ marginTop: '1rem' }}>
                        <a className="btn ghost small" href={j.result.mask_url} download>Download Mask (.nii.gz)</a>
                        <a className="btn ghost small" href={j.result.overlay_url} target="_blank" rel="noreferrer">View Overlay</a>
                      </div>
                      ...
                      {j.meta ? (
                        ...
                        <div className="muted small" style={{marginTop: '1rem'}}>Loading volume metadata…</div>
                      )}

                      <details style={{ marginTop: '1rem', cursor: 'pointer' }}>
                        <summary className="muted small" style={{userSelect:'none'}}>Raw Output Metrics</summary>
                    ...
                    j.status !== "FAILED" ? <div className="muted small" style={{marginTop: '0.5rem'}}>Segmentation pipeline is processing this case...</div> : null
                  )}
                </div>
              ))}
            </div>
